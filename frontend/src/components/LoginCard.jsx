...
export default function LoginCard({ onClose, onLoginSuccess }) {
    // Режимы: 'login' | 'register' | 'forgot_email' | 'forgot_timer' | 'new_password'
    ...
    const handleSubmit = (e) => {
        ...
        if (authMode === 'login' && isLoginValid) onLoginSuccess();
        if (authMode === 'register' && isRegisterValid) onLoginSuccess();
        ...
        if (authMode === 'new_password' && isNewPasswordValid) {
            // Пароль успешно изменен -> логинимся
            onLoginSuccess();
        }
    };
                    ...
                    {authMode === 'forgot_timer' && (
                        <>
                            ...
                            <div className="flex flex-col gap-6">
                                ...
                                {/* Кнопка для теста: имитация перехода по ссылке из письма */}
